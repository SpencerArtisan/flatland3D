...
class WorldSpec extends AnyFlatSpec with should.Matchers {
  ...
  it should "render cube faces identically from all 6 orthogonal views" in {
    // INVARIANT: All cube faces should render identically when viewed straight-on
    // under identical lighting conditions
    val cubeSize = 10.0
    val world = World.infinite
    val cube = TriangleShapes.cube(100, cubeSize)
    
    val renderings = InvariantTestHelpers.renderOrthogonalViews(
      world = world,
      shape = cube, 
      shapeId = 100,
      origin = Coord.ZERO,
